...
import { v4 } from 'uuid';
import {UserNotFound} from "./user-model";
...
export default class DemandsModel extends BaseModel {
    ...
    async update(id: string, options: UpdateOptions): Promise<Demand> {
        ...
        const set_string = Object.keys(options).map((field_name) => {
            args.push(options[field_name]);
            return `"${field_name}" = $${args.length}`;
        }).join(", ");

        args.push(id);

        const sql = `UPDATE "Demands" SET ${set_string} WHERE "id" = $${args.length};`;
        const result = await this.dao.execute(sql, args);
    ...
    async close(sender: string): Promise<undefined> {
