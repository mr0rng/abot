import { DemandsUpdateRequest, DemandsCreateResponse } from '@abot/api-contract/target/demands'
...
export default new Command<DemandsUpdateRequest, DemandsCreateResponse>(
    ...
    async (app: Application, request: DemandsUpdateRequest): Promise<DemandsCreateResponse> => {
        ...
        const user = await ensureAdminUser(app, session);

        try {
            return await demandsModel.update(id, options);
        } catch (e) {
