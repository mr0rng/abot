...
describe("Next demand switch demands scenario", () => {
    let session_key;
    let response;
    ...
    test("response demand id in answer", () => expect(response.response.id).toBe(demand2.id));
    ...
    describe("Demand should be updated", () => {
        let result;
        beforeAll(async () => {result = await demandModel.get(demand2.id)});
        test("should set sender", () => expect(result.sender).toBe(senderUser.id));
    });

    describe("Previous demand should be declined", () => {
        let result;
        beforeAll(async () => {result = await demandModel.get(demand.id)});
        test("should remove sender", () => expect(result.sender).toBeNull());
    });

    describe("Next should return eldest not declined demand", () => {
        beforeAll(async () => {
            response = await client.demands.next({session: session_key});
        });
        ...
        test("response code correct", () => expect(response.code).toBe(200));
        test("response status correct", () => expect(response.status).toBe("ok"));
        test("response demand id in answer", () => expect(response.response.id).toBe(demand3.id));
        test("response demand sender in answer", () => expect(response.response.sender).toBe(senderUser.id));
