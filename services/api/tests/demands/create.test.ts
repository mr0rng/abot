...
test('can create', async () => {
  ...
  const demand = await env.dao.executeOne(
    `SELECT * FROM "Demands" WHERE "id"=$1;`, [result.id]
  ) as Demand;
  const participant = await env.dao.executeOne(
    `SELECT * FROM "Participants" WHERE "demand"=$1;`, [result.id]
  ) as Participants;
