...
test('admin can update', async () => {
  const response = await env.client.demands.update({ ...adminSession, id: 'Location/Another', payload: {some: 'data'} });
  
  expect(response.payload).toStrictEqual({some: 'data'});
  expect(response.description).toStrictEqual('anthr lct');
  
  expect(
    await env.dao.executeOne(`SELECT "payload" FROM "Demands" WHERE id=$1;`, ['Location/Another'])
  ).toStrictEqual({'payload': {some: 'data'}});
});

test('participant can update', async () => {
  const response = await env.client.demands.update({ ...recipientSession, id: 'Location/Another', description: 'some descr' });
