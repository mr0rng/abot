import { Demands, Scenarios, Users, Participants } from '.';
...
test('user can send', async () => {
  const response = await env.client.messages.send(
    {...commonSession, demand: Demands[0].id, type: 'message', payload: {"may message": "here!"}}
  );
  await expect(
    env.dao.executeOne(`SELECT * FROM "Messages" WHERE "id" = $1;`, [response.id])
...
test('admin can send anywhere', async () => {
  const response = await env.client.messages.send(
    {...adminSession, demand: Demands[1].id, type: 'message', payload: {"may message": "here!"}}
  );
  await expect(
    env.dao.executeOne(`SELECT * FROM "Messages" WHERE "id" = $1;`, [response.id])
  ).resolves.toStrictEqual({
...
test('user can`t send if not participant', async () => {
  await expect(env.client.messages.send(
    {...commonSession, demand: Demands[1].id, type: 'message', payload: {"may message": "here!"}}
  )).rejects.toThrow('Forbidden');
});

test('admin can`t send if demand does not exists', async () => {
  await expect(env.client.messages.send(
    {...adminSession, demand: 'some strange id', type: 'message', payload: {"may message": "here!"}}
